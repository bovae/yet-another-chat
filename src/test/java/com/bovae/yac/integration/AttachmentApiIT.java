...
class AttachmentApiIT {
    ...
    void setUp() {
        ...
        room = roomService.getRoomById(roomService
                .createRoom("attach-room", "desc", RoomVisibility.PUBLIC, userA)
                .id());
    ...
    void uploadFile_toAnotherUsersMessage_returns403() throws Exception {
        ...
        MockMultipartFile file =
                new MockMultipartFile("file", "notes.txt", MediaType.TEXT_PLAIN_VALUE, "some notes".getBytes());
    ...
    void downloadFile_asMember_returns200WithContent() throws Exception {
        MockMultipartFile file =
                new MockMultipartFile("file", "download-me.txt", MediaType.TEXT_PLAIN_VALUE, "file content".getBytes());

        uploadThenDownloadAsUserA(file)
    ...
    void downloadImage_servedAsAttachmentNotInline() throws Exception {
        ...
        MockMultipartFile file = new MockMultipartFile("file", "pic.png", "image/png", png);
    ...
    void uploadFile_asNonMember_returns403() throws Exception {
        MockMultipartFile file =
                new MockMultipartFile("file", "secret.txt", MediaType.TEXT_PLAIN_VALUE, "secret data".getBytes());
    ...
    void downloadFile_fromPrivateRoomAsNonMember_returns403() throws Exception {
        ...
        Room privateRoom = roomService.getRoomById(roomService
                .createRoom("attach-private", "desc", RoomVisibility.PRIVATE, userA)
                .id());
        Message privateMsg = messageService.sendMessage(privateRoom, userA, "private msg", null);

        MockMultipartFile file =
                new MockMultipartFile("file", "private.txt", MediaType.TEXT_PLAIN_VALUE, "private content".getBytes());
    ...
    /**
     * Uploads {@code file} to {@link #room} as {@link #userA} (expecting 201), then downloads the resulting
     * attachment as {@link #userA}. Returns the download {@link ResultActions}, already asserted 200, so callers
     * can chain their own response-header expectations.
     */
    private ResultActions uploadThenDownloadAsUserA(MockMultipartFile file) throws Exception {
        mockMvc.perform(multipart("/api/rooms/{roomId}/attachments", room.getId())
                        .file(file)
                        .param("messageId", message.getId().toString())
                        .with(user(userA.getEmail()).roles("USER"))
        ...
        return mockMvc.perform(get("/api/rooms/{roomId}/attachments/{id}/download", room.getId(), attachment.getId())
                        .with(user(userA.getEmail()).roles("USER")))

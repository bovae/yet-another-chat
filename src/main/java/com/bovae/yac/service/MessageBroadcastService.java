...
public class MessageBroadcastService {
    ...
    public void broadcastEdit(Room room, UUID messageId, String content) {
        messagingTemplate.convertAndSend(
                "/topic/room." + room.getId(), MessageEditedEvent.of(messageId, room.getId(), content));
    ...
    private void fanOutUnread(Room room, @Nullable User excludeUser) {
        ...
        for (RoomMember member : members) {
            ...
            notificationService.broadcastNotification(
                    recipient, new NotificationEvent("UNREAD_UPDATE", room.getId(), room.getName(), unread));
        }
    }
}

...
- AFK status is set only when all open tabs have been inactive for more
  than 1 minute.

- A user becomes offline only when all browser tabs are closed/offloaded
  by browser as inactive.
...
from the **<u>current browser</u>** invalidates only the session for
that browser. Other active sessions remain valid until the user
...
- For sizing assumptions, a typical user may have around 20 rooms and 50
  contacts.
...
- The application shall work correctly across multiple tabs for the same
  user.
...
- A user is considered offline only when they have no open browser tabs
...
- Project MUST be buildable and runnable by \`docker compose up\` in the
